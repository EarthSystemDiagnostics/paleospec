...
#' @description estimates cross spectral densities using slepian tapers with
#'   adaptive weighting. Function is an adaptation of Peter Huybers Matlab
#'   coherence function cmtm.m
#' @param x,y input data vectors (or array if y=NULL)
#' @param dt time stepping (default = 1)
#' @param nw slepian bandwidth parameter, typically something between 2.0
#'   (default) and 8.0
#' @param k The number of Slepian tapers used, often (default = NULL). For
#'   default NULL (2*nw -1) is used except for the cases 2*nw > N
#' @param detrend (default=T)
#'
#' @return spectra object
#' @export
#' @examples
#' x <- SimPowerlaw(1, 1e03)
#' y <- ApplyFilter(x, filter = dnorm(-5:5, 0, 1), method  =3)
#' cs1 <- CrossSpecMTM(x, y)
#' plot(cs1$freq[1:length(x)], cs1$coh, log = "xy", type = "l")
#'
#' ch1 <- Coherence(x, y)
#' lines(ch1$freq, ch1$coh, col = "red")
#'
...
# how does nw in CrossSpecMTM fit to k,nw in SpecMTM?!
  ...
  for ( i1 in 1:2 ) {
    ...
    P1 <- Ptemp
    tol  <- .0005*vari[1,1]/N          # usually within tolerance in about three iterations
    a <-vari[1,1]*abs(1-V)
