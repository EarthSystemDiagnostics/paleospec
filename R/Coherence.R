...
Coherence <- function(x,y=NULL,dt=1,NW=8,k=NULL,dpssIN=NULL,detrend=T,qbias=0,confn=0) {
 ...
 if ( confn > 0 ) {
   ...
   phi=apply(phi,2,function(x) {quantile(x,probs = c(0.025,0.975))})
   phi=(phi[2,] + phi[1,]*-1)/2
   phi=filter(phi,filter=c(1,1,1)/3,method='convolution')
 ...
 if ( all(confn != 0) ) {
   ...
   c.conf <-
     sapply(conf.lev,function(y) {
       sapply(c,function(x) {CoherenceConf(v,lev=y,unbias=1,c=0)})
     })
...
CoherenceBias<-function(cb,dof) {
  ...
  ### The Estimation of the look-up-table seems to work. The results are equal to those of Huybers Matlab script.
  ### Nevertheless the provided lot looks different to the one estimated here...
  n <- seq(2,50)
  c <- seq(0.1,1,0.1)
