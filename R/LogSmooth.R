...
##' @export
LogSmooth<-function(spectra,df.log=0.05,removeFirst=1e6,removeLast=0,
                    bLog=FALSE)
{
...
weights<-function(x,sigma) 1/sqrt(2*pi*sigma^2)*exp(-x^2/(2*sigma^2))
...
smoothlog.cutEnd<-function(x,f,df.log,dof=1)
{
 ...
 ## Cut the weights down
     ...
     DistanceFastEnd<-length(f)-i
        ...
        if ((i+DistanceSlowEnd+1)<=length(f))  w[(i+DistanceSlowEnd+1):length(f)]<-0
        if ((i-DistanceFastEnd-1)>=1) w[1:(i-DistanceFastEnd-1)]<-0
        w<-w/f
